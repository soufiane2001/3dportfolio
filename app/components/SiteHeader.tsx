...
export default function SiteHeader() {
  return <header className="sticky top-0 z-50 border-b border-white/10 bg-black/92 backdrop-blur-xl"><div className="container flex min-h-20 items-center justify-between gap-5 py-3">
    <Link href="/" className="text-xl font-black text-white" aria-label="Soufiane Boutatss — Accueil">Soufiane<span className="text-[#ff6b00]">.</span></Link>
    ...
    <details className="relative xl:hidden"><summary className="cursor-pointer list-none rounded-lg border border-white/15 px-4 py-2 text-sm focus-visible:outline-2 focus-visible:outline-[#ff6b00]">Menu</summary><nav className="absolute right-0 mt-3 max-h-[75vh] w-80 overflow-y-auto border border-white/10 bg-[#080808] p-4 shadow-2xl">{[...serviceLinks, ...marketLinks, {href:"/portfolio",label:"Portfolio"},{href:"/blog",label:"Blog"},{href:"/a-propos",label:"À propos"},{href:"/contact",label:"Contact"}].map(link => <Link key={`${link.href}-${link.label}`} href={link.href} className="block px-3 py-2.5 text-sm text-white/75 hover:bg-white/5">{link.label}</Link>)}</nav></details>
  </div></header>;
}
